(function($){
    ...
    $(document).ready(function(){
		...
		$('#to-skills-section').click( function(event) {
		...
		function animate_sections() {
			...
			$('.wrap').waypoint(function(direction) {
				...
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					...
					setTimeout ( function () {
							$('#skills-section header').find('.section-title').animate({ opacity: 1 });
					}, 200 );
					
					setTimeout ( function () {
							$('#skills-section header').find('.section-slogan').animate({ opacity: 1 });
					}, 600 );
